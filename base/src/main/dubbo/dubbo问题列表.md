...
1. 
```
疑问：一般情况下，dubbo都会以集群形式部署（多个生产者），
   当发布时候至少会存在一台机子的服务是完整的，那么消费者调用时候，
   发现错误的情况下，可以重试调用。那么为什么还需要做服务的优雅下线操作？？
```  
```
   理解：  
   1. 一般情况下不希望消费者进行重试调用，正常来说重试会在超时情况下进行，那么此时多次重试可能服务端处理太慢
   导致超时，在进行重试，会加重服务端的消耗，甚至瘫痪，希望在可控范围内进行重试。
   2. 假设10台机子做生产者，发布时候，同时间发布5台，5台正常。那么当消费者调用到重启机子时候，会调用失败
   然后进行重试，那么下一次调用任然会重试到另外几台正在发布的机子，导致这次调用失败。
   3. 所以此时需要对生产者进行优雅下线操作，当发布生产者时，通知消费者，要下线了别来调用，从而
    消费者每次调用正常。
```
